...
def seq_to_token_ids(args, seq, candidate, label, text_dict, tokenizer, prompter, eval=False):
    ...
    can_t = truncate_title(text_dict[candidate])
...
class LLMTrainDataset(data_utils.Dataset):
    def __init__(self, args, seq_list, cand_list, label_list, max_len, rng, text_dict, tokenizer, prompter):
        self.args = args
        ...
        self.all_labels = []
        self.all_cands = []
        for seq, cand, label in zip(seq_list, cand_list, label_list):
            self.all_seqs.append(seq)
            self.all_cands.append(cand)
            label = 'Yes' if label >= 3 else 'No'
    ...
    def __getitem__(self, index):
        ...
        candidate = self.all_cands[index]
        label = self.all_labels[index]
        
        return seq_to_token_ids(self.args, seq, candidate, label, self.text_dict, \
                                self.tokenizer, self.prompter, eval=False)


class LLMValidDataset(data_utils.Dataset):
    def __init__(self, args, seq_list, cand_list, label_list, max_len, rng, text_dict, tokenizer, prompter, save_folder):
        self.args = args
        self.max_len = max_len
        self.rng = rng
        self.text_dict = text_dict
    ...
    def __getitem__(self, index):
        ...
        return seq_to_token_ids(self.args, seq, candidate, label, self.text_dict, self.tokenizer, self.prompter, eval=True)
